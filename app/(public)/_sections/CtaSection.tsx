    ...
    <>
      ...
      <section id="book" className="py-32 md:py-48 px-5 max-w-[1400px] mx-auto">
        ...
        >
          ...
          />
          {/* Inner edge refraction (Liquid Glass) */}
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <div className="mt-8 flex flex-wrap items-center gap-6 justify-center">
              {["بدون بطاقة ائتمان", "إلغاء مجاني في أي وقت", "دعم ٢٤/٧"].map((t) => (
                <span key={t} className="flex items-center gap-1.5 text-light/50 text-sm">
      ...
      <footer className="border-t border-dark/8 px-5 py-14 max-w-[1400px] mx-auto w-full">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          <div>
            <p className="text-primary font-bold text-2xl mb-1 tracking-tight">{ar.footer.brand}</p>
            <p className="text-dark/45 text-sm">{ar.footer.tagline}</p>
          </div>
          <nav className="flex flex-wrap gap-x-8 gap-y-2">
            {[
              ar.footer.links_services,
              ar.footer.links_about,
              ar.footer.links_privacy,
              ar.footer.links_terms,
            ].map((label) => (
              <Link
                key={label}
                href="#"
        ...
        <div className="mt-8 pt-6 border-t border-dark/6 text-center">
          <p className="text-dark/30 text-xs">{ar.footer.copyright}</p>
        </div>
      </footer>
    </>
  );
