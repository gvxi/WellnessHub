...
import PosModeDock from "@/app/(admin)/zw0/dashboard/_components/PosModeDock";
import PosBookingsTab from "../dashboard/_sections/PosBookingsTab";
import PosStorefront from "./PosStorefront";
...
export default function PosModeClient({ permissions, employeeEmail }: Props) {
  const router = useRouter();
  const { t, isRTL } = useLang();
  const [activeTab, setActiveTab] = useState<Tab>("services");
  const [coverClip, setCoverClip] = useState<string | null>(null);
  const [exitRipple, setExitRipple] = useState<{ x: number; y: number } | null>(null);
  const navTimer = useRef<ReturnType<typeof setTimeout> | null>(null);

  const canServices = (permissions.tab_services as TabPermission) !== "hidden";

  // Entry: circle-collapse reveal from button origin
  useEffect(() => {
    const origin = readOrigin();
    setCoverClip(`circle(200vmax at ${origin.x}px ${origin.y}px)`);
    const id = requestAnimationFrame(() =>
      setCoverClip(`circle(0px at ${origin.x}px ${origin.y}px)`)
    );
  ...
  const TABS: { id: Tab; icon: typeof CalendarDays; label: string; show: boolean }[] = [
    { id: "bookings", icon: CalendarDays, label: t("admin.tab_bookings"), show: true },
    { id: "services", icon: Layers,       label: t("admin.tab_services"), show: canServices },
  ];
